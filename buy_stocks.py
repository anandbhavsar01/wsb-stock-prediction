...
def equity_model_ts_one_stock(stocks, sentiment_data, ticker_data):
            ...
            for stock in stocks.keys():
                if sentiment.loc[day,stock] > 0:
                    ...
                    if stocks[stock] > 0:
                        ...
                        stocks[stock] = 0
                equity += ticker_data.loc[ticker_data['Day']==int(sentiment.loc[day,stock]['Day'])].values[0]
                # else hold
            ...
            for stock in stocks.keys():
                if sentiment.loc[day,stock] > 0:
                    ...
                    if stocks[stock] >= sentiment.loc[day,stock]:
                        ...
                        stocks[stock] = 0
                equity += stocks[stock] * ticker_data.loc[ticker_data['Day']==int(sentiment.loc[day,stock]['Day'])].values[0]
                # else hold
                equity_dict[day] = equity
                holding_dict[day] = stocks
            print(equity, stocks)
...
sentiment_data =  pd.read_csv('dataset/sentiment_ticker_by_day_sum.csv').shift(1)
#sell_at_end_model(stocks, sentiment_data, ticker_data)
...
equity_dict, holding_dict = equity_model_ts_one_stock(stocks, sentiment_data.fillna(0), ticker_data)
equity_df_one_stock  = pd.DataFrame(equity_dict, index = ['Portfolio Value']).T
holding_df_one_stock  = pd.concat(holding_dict)